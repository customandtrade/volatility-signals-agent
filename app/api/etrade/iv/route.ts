...
/**
 * GET /api/etrade/iv - Get historical IV
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const historicalIV = await client.getHistoricalIV(symbol, days);
    
    return NextResponse.json({ iv: historicalIV });
  } catch (error: any) {
    console.error('E*TRADE IV error:', error);
    return NextResponse.json(
